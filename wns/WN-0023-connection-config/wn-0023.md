...
Modes interact naturally with persistence (see WN-0022):

- Each mode may produce different manifests because connection digests may differ
- BuildIDs are content-addressed: if two modes resolve to the same effective database, the dialect can produce the same connection digest, and manifest entries are interchangeable
- Applications can configure persistence behavior (like `strictPersist`) per mode as an app-level setting, not in this file

### Manifest Layout

Tools that work with persistence manifests on the local filesystem (VSCode, builder CLI) use a convention of `.manifest/` directories alongside model files:
...
Given a model file at `<dir>/<name>.malloy` and an active mode `<mode>`, the manifest is at `<dir>/.manifest/<mode>/<name>.manifest.json`. Manifests travel with their models if directories are reorganized.

This is a convention between tools, not a requirement. Other applications may store manifests differently (URLs, artifact stores, APIs).
...
- **Packages**: A future section could describe how a set of models exports or imports model packages.
