...
> Note: Not sure what query statements are legal in these queries, maybe query syntax isn't right for these gestures because there are only two "statements", filter, and value computation, so the syntax could be simpler. Something like `map(array, mapExpr, [ filter ])`, and `reduce(array, reduceExpr [ filter ])`
