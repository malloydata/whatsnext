...
> Note: No help in finding last element of array other than length(array)-1, objections exist to using some sigil and to using -1.
...
| `slice(array,x)` | The array starting at index `x`, to the end |
| `slice(array, x, y)` | The array starting at index `x`, ending at index `y` |
| `span(array,x, l)` | The array starting at index `x`, for `l` characters |
| `at_index(x)` | The element at index `x`, -1 is the end of the string (-2 one before end, etc. ) |
